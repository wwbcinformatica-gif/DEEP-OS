@echo off
chcp 65001 >nul 2>&1
title Instalar Dependencias - DEEP-AUREA
color 0A

echo ============================================
echo   INSTALAR DEPENDENCIAS - DEEP-AUREA
echo ============================================
echo.
...
"%PROJECT_DIR%venv\Scripts\python.exe" -c "import sys; print(f'Python: {sys.version}'); print(f'Bits: {\"64-bit\" if sys.maxsize > 2**32 else \"32-bit\"}')"
...
"%PROJECT_DIR%venv\Scripts\python.exe" -c "import openai; print(f'  openai: {openai.__version__}')"

echo.
echo Verificando GPU (nvidia-smi)...
"%PROJECT_DIR%venv\Scripts\python.exe" -c "import subprocess; r = subprocess.run(['nvidia-smi', '--query-gpu=name,memory.total', '--format=csv,nounits,noheader'], capture_output=True, text=True, timeout=5); print(f'  GPU: {r.stdout.strip()}' if r.returncode == 0 else '  GPU: nao detectada (normal se nao tiver GPU NVIDIA)')"
...
echo ============================================
echo.
echo Para iniciar o DEEP-AUREA:
echo   C:\DEEP-AUREA\START-TOTAL.bat
echo.
