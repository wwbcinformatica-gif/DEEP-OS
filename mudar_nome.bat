...
:found
echo ==========================================
echo  DEEP-AUREA - Mudar Nome do Assistente
echo ==========================================
echo.
echo  Projeto: %BASE%
echo  Nome atual: Charon
echo  (digite "charon" para voltar ao original)
echo.
set /p "novo_nome=Novo nome: "
...
if os.path.exists(api_file):
    ...
    data['voice_preset'] = NEW.lower()
    json.dump(data, open(api_file, 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
    print(f'  [OK] api_keys.json')
    count += 1

print(f'\nFeito! {count} arquivos atualizados para {NEW}.')
" "%novo_nome%" "%BASE%"

echo.
echo ==========================================
echo  Nome alterado para: %novo_nome%
echo ==========================================
echo.
